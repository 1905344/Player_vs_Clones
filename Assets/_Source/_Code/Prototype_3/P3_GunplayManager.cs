...
public class P3_GunplayManager : MonoBehaviour
{
    ...
    public void DisableGun()
    {
        ...
        Debug.Log("P3_GunplayManager: Disabling the gun!");
    }
    ...
    private void GunInput()
    {
        ...
        {
            isShooting = P3_InputManager.Instance.IsPlayerHoldingTheFireButton;
        }
        else if (!allowFireButtonHold)
        {
            isShooting = P3_InputManager.Instance.IsPlayerTappingTheFireButton;
        }
    ...
    private void FireGun()
    {
        ...
        if (Physics.Raycast(_camera.transform.position, spreadDirection, out _raycastHit, bulletRange, detectionMask))
        {
            ...
            if (P3_GameManager.Instance.enableDebug)
            {
                ...
                Debug.Log("P3_GunplayManager: Gun fired!");
                Debug.Log("P3_GunplayManager: Bullet hit: " + _raycastHit.collider.name);
            }
            ...
            if (_raycastHit.collider != null && _raycastHit.collider.CompareTag("Enemy"))
            {
                ...
                if (P3_GameManager.Instance.enableDebug)
                {
                    Debug.Log("P3_GunplayManager: Hit an enemy! ID is: " + enemyGuid);
                }
                ...
                if (_raycastHit.collider.name == "Enemy Head")
                {
                    ...
                    P3_GameManager.Instance.OnEnemyHit(enemyGuid, headShotDamage);

                    #region Debug

                    if (P3_GameManager.Instance.enableDebug)
                    {
                        Debug.Log("P3_GunplayManager: Hit enemy " + enemyGuid + "'s head!");
                    }
                ...
                {
                    P3_GameManager.Instance.OnEnemyHit(enemyGuid, bulletDamage);
                }
            }
                ...
                || _raycastHit.collider.CompareTag("Environment") || _raycastHit.collider.CompareTag("Pushable"))
            {
                ...
                if (P3_GameManager.Instance.enableDebug)
                {
                    Debug.Log($"P3_GunplayManager: Hit {_raycastHit.collider.tag}. Spawning a bullet decal.");
                }
        ...
        P3_CinemachineShake.Instance.ShakeCamera(cameraShakeMagnitude, cameraShakeAmplitude);
    ...
    private void GunRecoil()
    {
        ...
        if (P3_GameManager.Instance.enableDebug)
        {
            Debug.Log("P3_GunplayManager: Gun recoil function has been called.");
        }

        #endregion

        //P3_GameManager.Instance.OnGunFired();
        isGunRecoiling = true;
    }
    ...
    private void Update()
    {
        if (isActiveGun)
        {
            ...
            if (isShooting && !isReloading && bulletsRemaining == 0 && canShoot)
            {
                if (P3_GameManager.Instance.enableReloadPromptTextAsTimer)
                {
                    P3_GameManager.Instance.ShowReloadPrompt();
