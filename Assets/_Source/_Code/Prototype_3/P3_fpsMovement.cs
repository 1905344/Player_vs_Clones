...
public class P3_fpsMovement : MonoBehaviour
{
    #region Variables
    ...
    private void Update()
    {
        ...
        if (!disablePlayerMovement)
        {
            playerMovement = P3_InputManager.Instance.GetPlayerMovement();

            characterMove = new Vector3(playerMovement.x, 0f, playerMovement.y);

            //Sprinting
            isSprinting = P3_InputManager.Instance.isPlayerSprintingThisFrame;
        ...
        if (!disablePlayerJumping)
        {
            if (P3_InputManager.Instance.PlayerJumped() && onGround && canJump)
            {
                canJump = false;
                isJumping = true;
